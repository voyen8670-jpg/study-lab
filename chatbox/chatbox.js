(async function () {
  ...
  const wrap = $("#sl-wrap"),
    bd = $("#sl-bd"),
    t = $("#sl-t");
  function add(text, who = "a") {
    const d = document.createElement("div");
    d.className = "bub " + (who === "u" ? "u" : "a");
    d.textContent = text;
    bd.appendChild(d);
  ...
  function section(title, arr, bullet = "•") {
  ...
  function reply(q) {
    ...
    if (!it)
      return `Mình chưa có nội dung soạn sẵn cho chủ đề này trong “${EXP}”. Bạn thử mô tả rõ hơn hoặc chọn thí nghiệm khác.`;
  ...
  box.querySelector("#sl-s").onclick = () => {
    ...
    add(q, "u");
    t.value = "";
    setTimeout(() => add(reply(q), "a"), 180);
  };
  t.addEventListener("keydown", (e) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      box.querySelector("#sl-s").click();
    }
  });

  /* 6) Lời chào */
  add(
    "Xin chào! Mình sẽ hướng dẫn THAO TÁC và GIẢI THÍCH cho thí nghiệm hiện tại."
  );
})();
